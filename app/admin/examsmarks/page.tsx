      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden">
        ...
        {marksData.length > 0 ? (
          <>
                ...
                <tbody className="divide-y divide-slate-50">
                  {marksData.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="p-6">
                        <p className="font-black text-slate-700 text-xs uppercase">
                          {row.name}
                        </p>
                        ...
                        {!row.is_locked && (
                          <button
                            ...
                            className="text-[9px] font-black text-[#a63d93] uppercase hover:underline mt-1"
                          >
                            {row.status === 'Absent'
                              ? 'Mark Present'
                              : 'Mark Absent'}
                      ...
                      <td className="p-6 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            updateRow(idx, 'marks_obtained', e.target.value)
                          }
                          className="w-20 p-2 text-center border-2 rounded-xl font-black outline-none border-[#a63d93]/20"
                          placeholder="--"
                        />
                      </td>
                      ...
                      <td className="p-6 text-center">
                        <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase border ${row.status === 'Pass'
            ...
            <div className="md:hidden p-4 space-y-4">
              {marksData.map((row, idx) => (
                <div
                  key={row.student_id}
                  className={`rounded-3xl border p-5 shadow-sm space-y-4 ${row.status === 'Pending'
                      ? 'bg-[#fdf2f9]/40 border-[#a63d93]/10'
                      : row.is_locked
                ...
                >
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-3 py-1 text-[9px] rounded-full font-black uppercase border ${row.status === 'Pass'
                        ? 'bg-green-50 text-green-600 border-green-100'
                        : row.status === 'Fail'
                          ? 'bg-red-50 text-red-600 border-red-100'
                          : row.status === 'Absent'
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="mt-2 w-full p-3 text-center border-2 rounded-2xl font-black outline-none border-[#a63d93]/20"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="mt-2 w-full p-3 border rounded-2xl text-xs outline-none focus:border-[#a63d93]"
                  ...
                  {!row.is_locked && (
                    <button
                      onClick={() => toggleAbsent(idx)}
                      className="w-full py-2 text-xs font-black uppercase rounded-xl bg-[#fdf2f9] text-[#a63d93]"
                    >
                      {row.status === 'Absent'
                        ? 'Mark Present'
                        : 'Mark Absent'}
          ...
          <div className="p-24 text-center">
            <p className="font-black text-slate-300 uppercase text-xs tracking-[0.3em]">
              No Selection
            </p>
          </div>
        )}
