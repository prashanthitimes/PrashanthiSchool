...
export default function AdminStudentDirectory() {
    ...
    const fetchData = useCallback(async () => {
        ...
        const { data, error } = await supabase
            .from('students')
            .select(`*, parents(email, temp_password, full_name)`)
            .order('class_name', { ascending: true })

        if (data) setStudents(data)
        if (error) toast.error("Failed to load database")
        setLoading(false)
    }, [])
            ...
            <header className="flex flex-col lg:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        onClick={() => { 
                            setEditMode(false); 
                            setFormData({id:'', student_id:'', full_name:'', parent_phone:'', class_name:'', section:'', roll_number:'', status:'active', academic_year:'2026-27'}); 
                            setIsModalOpen(true); 
                        }}
                        className="bg-brand-light text-white px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-brand-soft hover:brightness-110 transition-all flex items-center gap-2"
                    >
                    ...
                    <tbody className="divide-y divide-brand-soft/10">
                        {loading ? (
                            ...
                            <tr key={stu.id} className="hover:bg-brand-soft/5 transition-all group">
                                ...
                                <td className="px-8 py-6">
                                    {stu.parents && (Array.isArray(stu.parents) ? stu.parents.length > 0 : !!stu.parents) ? (
                                        ...
                                        <div className="text-slate-400 font-bold text-[10px] uppercase tracking-tighter italic">Pending Link</div>
                                    )}
                                </td>
                                <td className="px-10 py-6">
                                    <div className="flex justify-center gap-2">
                                        ...
                                        <button onClick={() => { setEditMode(true); setFormData({...stu}); setIsModalOpen(true); }} className="p-3 bg-slate-100 text-slate-500 rounded-xl hover:bg-slate-800 hover:text-white transition-all"><FiEdit3 size={16}/></button>
                                        <button onClick={() => handleDelete(stu.id)} className="p-3 bg-rose-50 text-rose-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><FiTrash2 size={16}/></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95">
                        <div className="p-8 border-b border-brand-soft bg-brand-soft/10 flex justify-between items-center">
                            <h2 className="text-xl font-black text-brand-light uppercase tracking-tighter">{editMode ? 'Update' : 'Enroll'} Student</h2>
                        ...
                        <form onSubmit={handleSave} className="p-10 space-y-6">
                            <div className="grid grid-cols-2 gap-5">
                                <div className="col-span-2">
                                    ...
                                    <input required value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold border-2 border-transparent focus:border-brand-soft outline-none" />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase">Grade</label>
                                    <select required value={formData.class_name} onChange={e => setFormData({...formData, class_name: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <select required value={formData.section} onChange={e => setFormData({...formData, section: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none cursor-pointer">
                                        <option value="">Select</option>
                                        {sections.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase">Roll No</label>
                                    <input required type="number" value={formData.roll_number} onChange={e => setFormData({...formData, roll_number: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-brand-soft"/>
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase">Phone</label>
                                    <input required value={formData.parent_phone} onChange={e => setFormData({...formData, parent_phone: e.target.value})} className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-brand-soft" />
                                </div>
                            </div>
                            <button disabled={saving} className="w-full bg-brand-light text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-xl shadow-brand-soft hover:brightness-110 active:scale-[0.98] transition-all">
                                {saving ? "PROCESSING..." : "SAVE STUDENT"}
                            </button>
                        </form>
                    </div>
                </div>
