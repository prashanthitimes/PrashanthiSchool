...
export default function SettingsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setYearStart(data.academic_start_year?.toString() || "");
      setYearEnd(data.academic_end_year?.toString() || "");
    ...
    <div className="max-w-8xl mx-auto mt-10 px-4 sm:px-6 lg:px-8 py-4 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-8">
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-3 bg-white/10 p-2 rounded-2xl backdrop-blur-sm border border-white/20">
                ...
                <input
                  type="number"
                  value={yearEnd}
                  onChange={e => setYearEnd(e.target.value.slice(0, 4))}
                  className="w-full p-3 bg-white dark:bg-slate-800 rounded-xl outline-none font-black text-center text-xl text-brand-dark dark:text-white shadow-inner"
                />
              </div>
            </div>
          ...
          <section className="bg-white dark:bg-slate-900 p-6 sm:p-10 rounded-[2.5rem] border border-brand-soft dark:border-slate-800 shadow-sm transition-colors">
            <div className="flex items-center gap-4 mb-8">
              <div className="p-3 bg-brand-soft-bg dark:bg-slate-800 rounded-2xl text-brand-light border border-brand-soft dark:border-slate-700">
                <FiGlobe size={22} />
              </div>
              <h2 className="text-sm font-black uppercase text-slate-800 dark:text-slate-100 tracking-widest">
                Social Media Links
              </h2>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <FormItem label="YouTube Link">
                <input
                  ...
                  className="w-full p-4 bg-brand-soft-bg/30 dark:bg-slate-800/50 border-2 border-brand-soft/50 dark:border-slate-700 rounded-2xl outline-none font-bold text-sm"
                />
              </FormItem>
