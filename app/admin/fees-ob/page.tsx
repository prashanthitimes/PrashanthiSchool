...
export default function FeesOpeningBalanceRegistry() {
  // Primary States
  const [obRecords, setObRecords] = useState<any[]>([]);
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      // Fetch school system configurations alongside base ledgers
      await Promise.all([
    ...
    try {
      ...
      if (!error && data) {
        if (data.academic_start_year && data.academic_end_year) {
          // Formats full numbers (e.g., 2026 and 2027) into your standard '2026-27' string format
          const endYearShort = String(data.academic_end_year).slice(-2);
          const formattedYear = `${data.academic_start_year}-${endYearShort}`;
          setSystemAcademicYear(formattedYear);
        }
      }
    } catch (err) {
  ...
  const fetchObRecords = async () => {
    ...
    if (!error && data) {
      setObRecords(data);
      const totalAmount = data.reduce((sum, item) => sum + Number(item.opening_balance || 0), 0);
      setTotals({
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      academic_year: systemAcademicYear, // Automatically injected from your school settings state
      opening_balance: parseFloat(formData.opening_balance),
      description: formData.description
    };

    const toastId = toast.loading(editRecord ? "Adjusting system matrix balance..." : "Registering ledger assets...");
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") {
          toast.error("Process Terminated", { description: `An OB account already maps to this student profile for ${systemAcademicYear}.`, id: toastId });
        } else {
  ...
  const handleDelete = async () => {
    ...
    const toastId = toast.loading("Purging transaction ledger balances...");
    try {
      const { error } = await supabase.from("student_fees_ob").delete().eq("id", targetRecord.id);
      if (error) {
        toast.error("Action Refused", { description: error.message, id: toastId });
      } else {
        toast.success("Ledger profile stripped cleanly", { id: toastId });
        setShowDeleteModal(false);
        fetchObRecords();
      }
    } catch (err) {
      toast.error("System structural error detected during standard teardown.", { id: toastId });
    }
  };
    ...
    <div className="max-w-8xl mx-auto mt-4 sm:mt-10 px-4 sm:px-6 lg:px-8 py-4 space-y-6 sm:space-y-8 animate-in fade-in duration-500">
      ...
      <header className="flex flex-col sm:flex-row items-center justify-between bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-6 py-6 sm:px-8 sm:py-6 rounded-[2rem] border border-brand-soft dark:border-slate-800 shadow-sm gap-4">
        ...
        <button onClick={() => handleOpenModal()} className="w-full sm:w-auto bg-brand-light hover:bg-brand text-white px-6 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-brand-soft">
          <Plus size={18} className="inline mr-2" /> Assign OB ({systemAcademicYear})
        </button>
      </header>

      {/* AGGREGATION BLOCKS */}
      <div className="grid grid-cols-2 gap-2 sm:gap-6">
        <StatCard title="Profiles Managed" value={totals.totalRecords} icon={<GraduationCap size={18} />} />
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-brand-soft/30 dark:bg-slate-800/50">
              <tr>
                <th className="p-6 text-[10px] font-black text-brand-light uppercase tracking-widest">Student Information</th>
                <th className="p-6 text-[10px] font-black text-brand-light uppercase tracking-widest">Class Placement</th>
                <th className="p-6 text-[10px] font-black text-brand-light uppercase tracking-widest">Target Year</th>
                <th className="p-6 text-[10px] font-black text-brand-light uppercase tracking-widest">Carried Forward Balance</th>
            ...
            <tbody className="divide-y divide-brand-soft/40 dark:divide-slate-800">
              {obRecords.length === 0 ? (
                ...
                <tr key={rec.id} className="hover:bg-brand-soft/10 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="p-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-wider font-mono">ID: {rec.student?.student_id || "N/A"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-6 align-middle font-bold text-slate-600 dark:text-slate-300 text-sm">
                    {rec.student ? `Class ${rec.student.class_name} - Sec ${rec.student.section}` : "Unallocated Data"}
                  </td>
                  ...
                  <td className="p-6 align-middle">
                    <p className="font-black text-red-500 text-sm">₹{Number(rec.opening_balance).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          ...
          {obRecords.length === 0 ? (
            ...
            <div key={rec.id} className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">ID: {rec.student?.student_id}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-50 dark:border-slate-800/60">
                <div>
                  <p className="text-[9px] text-slate-400 font-bold uppercase leading-none">Outstanding Carry-Over</p>
                  <p className="text-sm font-black text-red-500 mt-1">₹{Number(rec.opening_balance).toLocaleString('en-IN')}</p>
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-t-[2.5rem] sm:rounded-[3rem] w-full max-w-4xl shadow-2xl overflow-hidden border border-brand-soft dark:border-slate-800 flex flex-col h-[85vh] sm:h-auto sm:max-h-[90vh]">
            ...
            <div className="p-6 sm:p-8 border-b border-brand-soft dark:border-slate-800 flex justify-between items-center bg-brand-soft/20 dark:bg-slate-800/50">
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">{editRecord ? "Modify Ledger Adjustments" : "Initialize Historical Carry Forward"}</h2>
                <p className="hidden sm:block text-[10px] text-brand-light dark:text-brand-soft font-black uppercase tracking-[0.3em] mt-1">Auto-assigned to current system year: {systemAcademicYear}</p>
              </div>
            ...
            <div className="p-6 sm:p-8 space-y-6 sm:space-y-8 overflow-y-auto flex-1 custom-scrollbar">
              ...
              {!editRecord ? (
                      ...
                      {matchedStudentFilteringList.map((st) => (
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[10px] font-medium text-slate-400 mt-0.5">UID: {st.student_id} | Class Layout: {st.class_name} {st.section}</p>
                          </div>
                          <span className="text-[9px] font-black bg-brand-soft text-brand-light dark:bg-slate-800 px-3 py-1.5 rounded-xl uppercase">Map Profile</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="p-4 bg-brand-soft/20 dark:bg-slate-800/40 border border-brand-soft/40 dark:border-slate-700 rounded-2xl">
                  ...
                  <p className="text-[11px] font-semibold text-slate-400 mt-0.5 font-mono">System Record Key: {editRecord.student?.student_id}</p>
                </div>
              )}

              {/* OUTSTANDING METRICS LAYOUT BLOCKS */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <InputGroup label="Opening Arrears Amount (INR) *" icon={<IndianRupee size={16} />} error={errors.opening_balance}>
                  <input
                    type="number"
                    step="0.01"
              ...
              <InputGroup label="Log Entry Notes / Context Description" icon={<FileText size={16} />}>
                <textarea
                  ...
                  placeholder="Optional notes referencing historical payment failures or partial recovery agreements..."
                />
              </InputGroup>
            </div>
            ...
            <div className="p-6 bg-brand-soft/20 dark:bg-slate-800 border-t border-brand-soft dark:border-slate-700 flex flex-col sm:flex-row justify-end gap-3 sm:gap-4">
              ...
              <button onClick={handleSubmit} className="order-1 sm:order-2 bg-brand-light text-white w-full sm:w-auto px-10 py-4 rounded-2xl font-black uppercase tracking-widest text-[11px]">
                {editRecord ? "Modify Open State" : "Commit Financial Entry"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-brand-dark/60 dark:bg-slate-950/80 backdrop-blur-md z-[150] flex items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl border border-brand-soft dark:border-slate-800 overflow-hidden relative">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div>
                <h3 className="text-xl font-black tracking-tight uppercase dark:text-slate-100">Purge Open Balance?</h3>
                <p className="text-[11px] font-bold text-slate-400 dark:text-slate-500 mt-2 px-4 leading-relaxed">
                  You are stripping the historical balance for <span className="text-red-500">{targetRecord?.student?.full_name}</span>. This removes ledger records irreversibly.
                </p>
              </div>
              <div className="flex flex-col w-full gap-3 pt-4">
                <button onClick={handleDelete} className="w-full bg-red-500 text-white py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all active:scale-95 shadow-lg shadow-red-200">Confirm Deletion</button>
