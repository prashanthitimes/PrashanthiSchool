          ...
          <div className="p-6 md:p-8 border-t border-slate-100 flex flex-col md:flex-row md:flex-wrap gap-4 md:gap-12 bg-slate-50/30 shrink-0">
            <div className="flex justify-between md:block">
              <div>
                ...
                <p className="text-sm md:text-xl font-bold text-slate-700">₹{details?.records.reduce((a: number, b: any) => a + b.standard, 0).toLocaleString()}</p>
              </div>
              <div className="text-right md:text-left md:mt-0">
                ...
                <p className="text-sm md:text-xl font-bold text-emerald-600 md:hidden">₹{details?.records.reduce((a: number, b: any) => a + b.paid, 0).toLocaleString()}</p>
              </div>
            </div>

            <div className="hidden md:block">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Paid</p>
              <p className="text-xl font-bold text-emerald-600">₹{details?.records.reduce((a: number, b: any)=> a + b.paid, 0).toLocaleString()}</p>
            </div>
