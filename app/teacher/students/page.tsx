                                    ...
                                    {loading ? (
                                        ...
                                        <tr key={stu.id} className="hover:bg-brand-soft/5 transition-colors group">
                                            ...
                                            <td className="px-8 py-6 text-right">
                                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wider ${stu.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-50 text-rose-600'
                                                    }`}>
                                                    {stu.status}
                                                </span>
                                            </td>
                        ...
                        <div className="md:hidden divide-y divide-brand-soft/20">
                            {loading ? (
                                <div className="p-10 text-center">
                                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-brand mx-auto mb-4"></div>
                                    <p className="text-[10px] font-black text-brand-light uppercase tracking-widest">Loading...</p>
                                </div>
                            ) : filteredStudents.map((stu) => (
                                <div key={stu.id} className="p-5 active:bg-slate-50 transition-colors">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex gap-3">
                                            <div className="h-10 w-10 bg-brand-soft rounded-xl flex items-center justify-center text-brand font-black text-xs">
                                                {stu.full_name.charAt(0)}
                                            </div>
                                            <div>
                                                <p className="font-black text-slate-800 text-sm leading-tight">{stu.full_name}</p>
                                                <p className="text-[10px] font-bold text-brand-light uppercase tracking-tight">{stu.student_id}</p>
                                            </div>
                                        </div>
                                        <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg text-[9px] font-black">
                                            #{stu.roll_number || '00'}
                                        </span>
                                    </div>

                                    <div className="grid grid-cols-1 gap-3 bg-slate-50/50 p-3 rounded-2xl border border-slate-100">
                                        <div className="space-y-1.5">
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Parent Details</p>
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2 text-slate-600 font-bold text-[11px]">
                                                    <span className="w-3 h-3 rounded-full bg-brand-light/10 flex items-center justify-center text-[8px] text-brand">F</span>
                                                    {stu.father_name || '—'}
                                                </div>
                                                <div className="flex items-center gap-2 text-slate-600 font-bold text-[11px]">
                                                    <span className="w-3 h-3 rounded-full bg-brand-light/10 flex items-center justify-center text-[8px] text-brand">M</span>
                                                    {stu.mother_name || '—'}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="space-y-1.5 border-t border-slate-200/50 pt-2">
                                            ...
                                            <div className="flex items-center gap-2 text-slate-600 font-bold text-[11px] capitalize">
                                                <FiHome size={10} className="text-brand-light" />
                                    ...
                                    <div className="mt-4 flex justify-between items-center">
                                        <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-wider ${stu.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-50 text-rose-600'
                                            }`}>
                                            {stu.status}
                                        </span>
