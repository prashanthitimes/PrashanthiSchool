                ...
                <div className="relative overflow-hidden bg-brand-soft p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-brand-light/10 shadow-sm shadow-brand-soft/20">
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
                        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
                            <div className="p-4 md:p-5 bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-sm text-brand-light border border-brand-soft">
                                <FiLayers size={28} className="md:w-[32px] md:h-[32px]" />
                            </div>
                            <div>
                                <h1 className="text-2xl md:text-3xl font-black text-brand-dark tracking-tight">Syllabus Desk</h1>
                                <p className="text-brand-light font-bold text-xs md:text-sm mt-1 opacity-90">Organize exam portions for {todayName}.</p>
                            </div>
                        </div>
                        <div className="bg-white/60 backdrop-blur-md px-8 md:px-10 py-3 md:py-4 rounded-[1.5rem] md:rounded-[2rem] border border-white shadow-sm min-w-[140px] md:min-w-[160px]">
                            <p className="text-[9px] md:text-[10px] font-black text-brand-light uppercase tracking-widest text-center mb-1">Total Published</p>
                            <p className="text-3xl md:text-4xl font-black text-brand-dark text-center">{syllabusHistory.length}</p>
                        </div>
                    </div>
                    {/* Decorative Blurs */}
                    <div className="absolute -top-10 -right-10 w-32 md:w-40 h-32 md:h-40 bg-white/20 rounded-full blur-2xl"></div>
                </div>
            </div>

            <div className="px-4 md:px-6 mt-6 md:mt-8 grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 max-w-[1600px] mx-auto">

                {/* --- SIDEBAR / MOBILE HEADER: CLASSES --- */}
                <div className="lg:col-span-3 space-y-3">
                    <h2 className="text-[10px] md:text-[11px] font-black text-brand-light uppercase tracking-widest ml-2 md:ml-4 mb-2">My Assignments</h2>
                ...
                <div className="lg:col-span-9 space-y-6 md:space-y-8">
                    {selectedAlloc ? (
                        <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] border border-brand-soft p-5 md:p-8 shadow-sm relative overflow-hidden">
                            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                                <div className="space-y-4 md:space-y-5">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <h3 className="font-black text-brand-dark uppercase text-[10px] md:text-xs tracking-wider">New Entry Configuration</h3>
                                    </div>

                                    <div className="space-y-1">
                                        <label className="text-[9px] md:text-[10px] font-black text-brand-light uppercase ml-1">Select Active Exam</label>
                                        <select required value={selectedExamID} onChange={e => setSelectedExamID(e.target.value)}
                                            className="w-full bg-brand-accent/50 border-none rounded-xl p-3 text-sm font-bold text-brand-dark outline-none appearance-none">
                                    ...
                                    <div className="space-y-1">
                                        <label className="text-[9px] md:text-[10px] font-black text-brand-light uppercase ml-1">Add Chapter / Portion</label>
                                        <div className="flex gap-2">
                                            <input type="text" placeholder="e.g. Algebra" className="flex-1 bg-brand-accent/50 border-none rounded-xl p-3 text-sm font-bold placeholder:text-brand-light/40"
                                ...
                                <div className="flex flex-col justify-between pt-4 md:pt-0">
                                    <div className="bg-brand-accent/30 rounded-xl md:rounded-2xl p-4 border border-dashed border-brand-light/20 min-h-[100px] md:min-h-[120px]">
                                        <div className="flex flex-wrap gap-2">
                                            {chapterList.length === 0 && <p className="text-[10px] text-brand-light italic opacity-60 m-auto mt-8 md:mt-10 uppercase font-bold text-center w-full">No chapters added</p>}
                                            {chapterList.map((ch, idx) => (
                                                <span key={idx} className="bg-white text-brand-dark px-3 py-1.5 rounded-lg border border-brand-soft text-[10px] md:text-[11px] font-black flex items-center gap-2 shadow-sm">
                                                    {ch} <FiX className="cursor-pointer text-red-400" onClick={() => setChapterList(chapterList.filter((_, i) => i !== idx))} />
                                                </span>
                                            ))}
                                        </div>
                                    </div>
                                    <button disabled={loading || !selectedExamID} className="mt-4 w-full py-4 bg-brand text-white rounded-xl md:rounded-2xl font-black text-sm shadow-lg shadow-brand/20 hover:scale-[1.01] active:scale-95 transition-all disabled:opacity-30 uppercase tracking-widest">
                        ...
                        <div className="bg-brand-accent/20 border-2 border-dashed border-brand-soft rounded-[2rem] p-10 md:p-16 text-center">
                            <FiBookOpen size={40} className="mx-auto text-brand-light/30 mb-4" />
                            <p className="text-brand font-black text-xs md:text-sm uppercase tracking-widest">Select a class to start editing</p>
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-3 px-2">
                            ...
                            <h2 className="text-[10px] md:text-xs font-black text-brand-dark tracking-tight uppercase">Recently Published</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {syllabusHistory.map((item) => (
                                <div key={item.id} className="bg-white p-5 rounded-[1.5rem] md:rounded-[2rem] border border-brand-soft shadow-sm hover:shadow-md transition-all">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <span className="text-[8px] font-black text-brand bg-brand-accent px-2 py-0.5 rounded-full uppercase border border-brand-soft">
                                                {item.subjects?.name}
                                            </span>
                                            <h3 className="text-md md:text-lg font-black text-brand-dark mt-2 leading-tight">{item.exam_name}</h3>
                                            <p className="text-[10px] text-brand-light font-bold">Grade {item.class_name}-{item.section}</p>
                                        </div>
                                        <button onClick={() => handleDelete(item.id)} className="p-2 text-brand-light/30 hover:text-red-500 rounded-xl">
                                            <FiTrash2 size={16} />
                                        </button>
                                    </div>
                                    <div className="bg-brand-accent/30 p-3 rounded-xl mb-4">
                                        <p className="text-[10px] md:text-[11px] font-bold text-brand-dark leading-relaxed">
                                            {item.chapters.join(' • ')}
                                        </p>
                                    </div>
                                    <div className="flex items-center gap-2 text-[9px] md:text-[10px] font-black text-brand-light uppercase opacity-70">
