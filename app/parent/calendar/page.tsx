...
export default function AcademicCalendar() {
  ...
  return (
<div className="bg-[#FDFCFD] dark:bg-slate-950 p-4 md:p-10 pb-20">
...
{/* NAV */}
<div className="p-5 md:p-8 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900">

<h2 className="text-sm md:text-2xl font-black text-slate-800 dark:text-white uppercase">
...
<button
onClick={prevMonth}
className="p-2 md:p-3 hover:bg-[#722366] hover:text-white rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-700 transition-all"
>
<FiChevronLeft/>
</button>
...
className="px-3 md:px-4 py-2 text-[9px] md:text-[10px] font-black uppercase border border-slate-200 dark:border-slate-700 rounded-lg md:rounded-xl hover:bg-white dark:hover:bg-slate-800 dark:text-slate-200"
>
Today
</button>
...
<div key={d}
className="text-center text-[9px] md:text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase py-2">
{d}
</div>
))}
</div>
...
<div
key={idx}
className="aspect-square md:min-h-[100px] bg-slate-50 dark:bg-slate-800 rounded-lg md:rounded-2xl"
/>
)
...
aspect-square md:aspect-auto md:min-h-[60px]
p-1 md:p-2 rounded-lg border cursor-pointer transition-all flex flex-col

${isSelected
?'bg-[#722366] border-[#722366] text-white'
:'border-slate-100 dark:border-slate-700 hover:border-[#722366]/30'
}

${isToday&&!isSelected
?'ring-2 ring-[#722366] ring-offset-2 dark:ring-offset-slate-900'
:''
}
...
<div
className={`hidden md:block w-full text-[8px] font-black p-1 rounded-md truncate
...
<div className="md:hidden p-5 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
...
<div
key={ev.id}
className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex items-center gap-3"
>

<div className={`w-2 h-10 rounded-full ${typeColors[ev.event_type]?.split(' ')[0] || 'bg-slate-200'}`}/>
...
<div
key={type}
className="flex items-center gap-2 text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400"
>

<span className={`w-2 h-2 rounded-full ${typeColors[type].split(' ')[0]}`}></span>
...
<h2 className="text-sm md:text-lg font-black text-slate-800 dark:text-white uppercase flex items-center gap-2">
<FiTag className="text-[#722366]" />
Upcoming
</h2>
...
className="min-w-[250px] lg:min-w-full bg-white dark:bg-slate-900 p-5 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:border-[#722366] transition-all flex-shrink-0"
>

<span
className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase ${typeColors[event.event_type] || 'bg-slate-100'}`}
>
{event.event_type}
</span>

<h4 className="text-xs md:text-sm font-black text-slate-800 dark:text-white mt-2 uppercase">
{event.title}
</h4>
