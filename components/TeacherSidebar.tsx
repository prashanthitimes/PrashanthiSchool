...
import { 
  ...
  FiEdit3, FiFileText, FiUsers, FiBell, FiCheckSquare, FiMenu, FiX 
} from 'react-icons/fi'
...
export default function TeacherSidebar({ activeMenu, setActiveMenu }: Props) {
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 w-full h-16 bg-brand flex items-center justify-between px-4 z-[60] shadow-md">
        <div className="flex items-center gap-3">
            <img src="/Schoollogo.jpg" alt="Logo" className="w-8 h-8 rounded-lg bg-white p-1" />
            <span className="text-white font-black text-sm uppercase tracking-wider">Teacher Portal</span>
        </div>
        <button 
          onClick={() => setIsOpen(!isOpen)}
          className="text-white text-2xl p-2 bg-white/10 rounded-lg active:scale-95 transition-transform"
        >
          {isOpen ? <FiX /> : <FiMenu />}
        </button>
      </div>
      ...
      {/* SIDEBAR ASIDE */}
      <aside className={`
        fixed left-0 top-0 h-screen w-72 lg:w-64 bg-brand text-white flex flex-col shadow-2xl z-[58] overflow-hidden transition-transform duration-300 ease-in-out
      ...
      `}>
        ...
        {/* NAVIGATION */}
        <nav className="flex-1 overflow-y-auto py-4 px-3 scrollbar-thin scrollbar-thumb-white/10">
                  ...
                  {groupItems.map(({ id, label, icon, path }) => (
                    <Link key={id} href={path} className="w-full">
                      <button
                        ...
                        className={`relative flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm transition-all duration-200 group w-full ${
                          activeMenu === id 
                            ? 'bg-white text-brand shadow-xl font-black' 
